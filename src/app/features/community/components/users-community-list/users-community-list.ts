...
export class UsersCommunityList implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.translateService.get(['COMMON.ROLE.MEMBER', 'COMMON.ROLE.MANAGER', 'COMMON.ROLE.ADMIN']).subscribe(translations => {
      this.roles = [
  ...
  updateRole() {
    ...
    this.communityService.patchRoleUser({id_user: this.userSelected.id_user, new_role: this.roleSelected}).subscribe(
      {
        next:(response)=>
        {
          if (response) {
