...
import {AddressDTO, CreateAddressDTO} from '../../../../shared/dtos/address.dtos';
...
export class MeterUpdate implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.metersForm.patchValue({
      ...
      tarifGroup: this.tarifGroupCategory.find((tarifGroup) => tarifGroup.id === this.meter.tarif_group),
      phasesNumber: this.phaseCategory.find((phase) => phase.id === this.meter.phases_number),
      readingFrequency: this.readingFrequencyCategory.find((readingFrequency) => readingFrequency.id === this.meter.reading_frequency),
    });
